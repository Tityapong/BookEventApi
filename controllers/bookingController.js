...
const acceptBooking = async (req, res) => {
  try {
    ...
    const notificationMessage =
      "Your booking has been accepted by the supplier.";
    ...
    res.status(200).json({
      success: true,
      message:
        "Booking accepted successfully, and notification sent to the user.",
      data: {
        booking_id: id,
        status: "Confirmed",
      },
...
const rejectBooking = async (req, res) => {
  try {
    ...
    // Insert a notification for the user
    const notificationMessage =
      "Your booking has been rejected by the supplier.";
    const queryNotification = `
      INSERT INTO notifications (user_id, booking_id, message, is_read, created_at)
      VALUES (?, ?, ?, ?, NOW())
    `;
    ...
    res.status(200).json({
      success: true,
      message:
        "Booking rejected successfully, and notification sent to the user.",
      data: {
        booking_id: id,
        status: "Rejected",
      },
